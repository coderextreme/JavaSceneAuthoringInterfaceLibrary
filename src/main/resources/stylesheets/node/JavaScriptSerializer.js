...
JavaScriptSerializer.prototype = {
	serializeToString : function(json, element, clazz, mapToMethod, fieldTypes) {
		...
		bodystr += element.nodeName+stack[0]+" = Browser.createNode(\""+element.nodeName+"\");\n";
			...
			strval = this.printSubArray(attrType, "java.lang.String",
				...
				map(function(x) {
					var y = x.
					       ...
					       replace(/\\n/g, '\\n');
					if (y !== x) {
						// console.error("JavaScriptSerializer Replacing "+x+" with "+y);
					}
					return y;
				}), this.codeno, '","', '"', '"');
		} else if (
	...
	subSerializeToString : function(element, mapToMethod, fieldTypes, n, stack) {
		...
		for (var cn in element.childNodes) {
			...
			if (element.childNodes.hasOwnProperty(cn) && node.nodeType == 1) {
				...
				this.preno++;
				var ch = "";
				ch += node.nodeName+stack[0]+" = Browser.createNode(\""+ node.nodeName+"\");\n";
				...
				if (method.startsWith("add")) {
					ch += element.nodeName+stack[1]+"."+method.substring(4)+"("+node.nodeName+stack[0]+");\n\n";
